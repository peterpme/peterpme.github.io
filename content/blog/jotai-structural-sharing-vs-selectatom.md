...
The interceptor on `set` walks the previous value against the new one and splices old references in wherever they're structurally equal. After that, every plain derived atom downstream — `atom(g => g(users)[3].name)`, `atom(g => g(users).filter(u => u.active))` (still a fresh array, see above), `atom(g => g(users).length)` — gets the benefit for free.
...
It's worth asking. Two reasons, I think.
...
- Plain `atom(g => g(x).field)` for primitive or stable-reference slices.
- Two derived atoms instead of one `selectAtom` wrapping `{a, b}` whenever possible.
- `selectAtom + shallow` only for `.filter`/`.map` projections and composite values that must be atomic.
- For object state with named fields, split into per-field derived atoms before reaching for anything fancier.
- For list state, pick one: small list / few subscribers → one atom, no sharing; convenient writes + row-level dedupe → `atomWithStructuralSharing`; surgical updates from a normalized data layer → ids array + `atomFamily`.
- `atomWithStructuralSharing` at the source for any external data feed (sockets, manual fetches, anything not already doing it) where you've decided option 2 fits.
- If you're consuming `jotai-tanstack-query`, never `useAtomValue(rawQueryAtom)` directly. Derive the field you actually read.

That last one is what turned 44× into 1× in the benchmark. Same atom, same observer, same React Query underneath — just one derived atom in the way to give jotai an `Object.is` it can actually use.
