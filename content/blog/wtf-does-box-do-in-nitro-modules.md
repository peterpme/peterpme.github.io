...
`HostObject` is JSI's original escape hatch for native interop. You implement a `get()` method in C++, and JSI calls it whenever JavaScript accesses a property. Crucially, `HostObject` **does not use `NativeState`** — it's a plain C++ virtual interface that the worklet serializer already knows how to move across runtimes.
...
When `.unbox()` is called, it calls `hybridObject->toObject(runtime)` — passing in **the new runtime**. That's the `toObject` we saw earlier, which calls `setNativeState` on the current runtime. The result is a fully wired `HybridObject` rooted in the worklet runtime, backed by the same C++ instance.
...
The `'worklet'` directive at the top tells the worklet compiler to bundle this function into the worklet runtime's code. Functions without it can't be called from a worklet closure.
...
- **`Arc<Mutex<T>>` in Rust** — the wrapper (`Arc`) is what gets moved across threads, not the inner value directly.
